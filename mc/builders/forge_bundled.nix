{
  ...
  installCmd =
    ...
    '';
  serverDeps =
    if forgeInfo.includesInstaller
    then
  ...
  javaCmd =
    ...
    else "${temurinHeadless}/bin/java @user_jvm_args.txt @libraries/net/minecraftforge/forge/${mcVersion}-${forgeInfo.version}/unix_args.txt nogui";
